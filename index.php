 ...
 *
 * Provides help content, demonstrates pageview counter
 */
...
<body>
	  ...
	  <div class="row">
	    <section class='col-xs-12 col-sm-6 col-md-6'>
	      <section>
	        ...
	        <p>The source code for this application is available to be forked from the <a href="https://www.github.com/openshift/cakephp-ex">OpenShift GitHub repository</a>. You can configure a webhook in your repository to make OpenShift automatically start a build whenever you push your code:</p>
					<ol>
	...
	$ git push</pre>
	        ...
	        <p>In order to access the original CakePHP application, you have to remove the app/View/Layouts/default.ctp file which serves as the welcome page and rename the default.ctp.default to default.ctp.</p>
	        <p>It will also be necessary to update your application to talk to your database back-end. The <code>app/Config/database.php</code> file used by CakePHP was set up in such a way that it will accept environment variables for your connection information that you pass to it. 
	        	Once an administrator has created a MySQL database service for you to connect with you can add the following environment variables to your deploymentConfig to ensure all your frontend pods have access to these environment variables.</p>
	...
	oc env dc/cakephp-mysql-example DATABASE_PASSWORD=&lt;your database user's password&gt;
	        ...
	        <p>Note: If the database service is created in the same project as the frontend pod, the *_SERVICE_HOST and *_SERVICE_PORT environment variables will be automatically created.</p>
	        <p>You will need to redeploy your application in order to pick up the new environment variables.  You can force a deployment by running:
	<pre>
	oc deploy cakephp-mysql-example --latest
	    ...
	    <section class="col-xs-12 col-sm-6 col-md-6">
	      ...
	      <p>Page view count: <span class="code" id="count-value"><?php echo $page['views']; ?></span></p>
	    </section>
	  </div>
	 ...
	 * Update Pageviews
	 *
	 * Updates the pageview count text in the #count-value span.
	 * 
	 * @param object page
	 * @param object request
	 */
	function update_pageviews(page, request) {
    document.getElementById("count-value").innerHTML = page.views;
	}
	 ...
	 */
	// ------------------------------------------------------------------------
	ajax("/track_pageview.php?path=<?php echo $page['path'] ?>&t=" + Math.random(), "update_pageviews");
	</script>
</body>
</html>